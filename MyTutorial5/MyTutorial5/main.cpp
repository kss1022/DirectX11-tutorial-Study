...
HRESULT InitDevice()
{
	...
	// Load the Texture	https://m.blog.naver.com/sorkelf/220846504837
	//hr = D3DX11CreateShaderResourceViewFromFile(g_pd3dDevice, L"seafloor.dds", NULL, NULL, &g_pTextureRV, NULL);	if (FAILED(hr))
	if (FAILED(hr))
	return hr;
	...
	//U V X -> X Y X
	sampDesc.AddressU = D3D11_TEXTURE_ADDRESS_WRAP;		//WARP : 일반적으로 많이씀 0에서 1 넘어가면 반복
	sampDesc.AddressV = D3D11_TEXTURE_ADDRESS_WRAP;		//MIRROR :1 번 반복될떄마다 뒤집혀서 반복됨 ->이음매가 갈라져 보이지 않게 하기 위해
	sampDesc.AddressW = D3D11_TEXTURE_ADDRESS_WRAP;		//BORDER : 0에서 1넘어가면 마지막 픽셀로 채워준다
